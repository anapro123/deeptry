"""
ARBITRAGE BOT - HIGH SENSITIVITY
Catches even tiny arbitrage opportunities!
"""
...
DEFAULT_CONFIG = {
    'MIN_PROFIT_PERCENT': 0.01,        # MUCH LOWER - catches tiny spreads
    'MIN_PROFIT_USD': 0.001,           # $0.001 minimum profit
    'TRADING_FEE': 0.001,              # 0.1% fee (same)
    'BASE_CURRENCIES': ['USDT', 'BTC', 'ETH'],
    'SCAN_INTERVAL': 5,
    'MAX_OPPORTUNITIES_DISPLAY': 100,   # Show more opportunities
    
    'ALERTS': {
        'enabled': True,
        'min_profit_to_alert': 0.05,    # Alert on $0.05 profit
        'min_confidence_to_alert': 50,   # Lower confidence requirement
        'cooldown_seconds': 60,
    },
    ...
    async def scan(self):
        ...
        for pair, exchange_data in pairs.items():
            ...
            if net_spread > CONFIG['MIN_PROFIT_PERCENT']:
                ...
                # Check multiple trade sizes including VERY small ones
                trade_sizes = [5, 10, 25, 50, 100, 250, 500, 1000]
                for trade_size in trade_sizes:
                    ...
                    if net_profit > CONFIG['MIN_PROFIT_USD']:
                        ...
                        break  # Found one profitable size
        ...
        # Send alerts for new high-value opportunities
        for opp in self.opportunities[:5]:
            if opp['profit_usd'] > 0.01:  # Only alert on > $0.01 profit
                is_new = True
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 ARBITRAGE BOT</h1>
            <div class="subtitle">Ultra-sensitive - finds even tiny opportunities!</div>
            <div class="settings">
                Min Spread: {{ config.MIN_PROFIT_PERCENT }}% | Min Profit: ${{ config.MIN_PROFIT_USD }} | Fee: {{ config.TRADING_FEE*100 }}%
            </div>
        </div>
...
async def background_scanner():
    print("🔄 Scanner started (ULTRA SENSITIVE)...")
    while True:
        try:
            await engine.scan()
            if engine.opportunities:
                print(f"📊 Found {len(engine.opportunities)} opportunities")
        except Exception as e:
...
async def main():
    ...
    print("\n" + "="*70)
    print("🚀 ARBITRAGE BOT - ULTRA SENSITIVE")
    print("="*70)
    print("📊 Exchanges: 6 exchanges connected")
