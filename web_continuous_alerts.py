"""
ARBITRAGE BOT - CONTINUOUS ALERTS
Sends alerts for EVERY opportunity found, not just new ones
"""
...
DEFAULT_CONFIG = {
    ...
    'ALERTS': {
        'enabled': True,
        'min_profit_to_alert': 0.01,  # Lowered to $0.01
        'min_confidence_to_alert': 40,  # Lowered to 40%
        'cooldown_seconds': 30,  # Reduced cooldown
        'alert_on_every_scan': True,  # NEW: Send alerts every scan
    },
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            ...
            if 'alert_on_every_scan' not in config.get('ALERTS', {}):
                config['ALERTS']['alert_on_every_scan'] = True
            return config
    else:
...
# ============================================
# ALERT SYSTEM - UPDATED FOR CONTINUOUS ALERTS
# ============================================
...
class AlertSystem:
            ...
            if user.get('chat_id'):
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        ...
                        print(f"✅ Alert sent to {user.get('name', 'user')}")
    ...
    async def send_alert(self, opportunity):
        ...
        # COOLDOWN CHECK - Only prevent EXACT same opportunity spam
        key = f"{opportunity['pair']}_{opportunity['buy_exchange']}_{opportunity['sell_exchange']}"
        now = time.time()
        cooldown = CONFIG['ALERTS']['cooldown_seconds']
        
        # If alert_on_every_scan is True, only check cooldown for exact duplicates
        if CONFIG['ALERTS'].get('alert_on_every_scan', True):
            # Still respect cooldown for the EXACT same opportunity
            if key in self.sent_alerts and now - self.sent_alerts[key] < cooldown:
                return
            self.sent_alerts[key] = now
        else:
            # Old behavior - only send new opportunities
...
class ArbitrageEngine:
    ...
    async def scan(self):
        ...
        # SEND ALERTS - FOR EVERY OPPORTUNITY FOUND (with cooldown for duplicates)
        for opp in self.opportunities[:10]:  # Send alerts for top 10
        ...
        if self.scan_count % 3 == 0:
            print(f"📊 Scan {self.scan_count}: Found {len(self.opportunities)} opportunities")
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 ARBITRAGE BOT</h1>
            <div class="subtitle">Continuous alerts - Every opportunity detected is sent!</div>
            <div class="alert-status">🔔 Alerts: {{ 'ON' if config.ALERTS.enabled else 'OFF' }} | Cooldown: {{ config.ALERTS.cooldown_seconds }}s</div>
        </div>
...
async def background_scanner():
    print("🔄 Scanner started - CONTINUOUS ALERTS MODE")
    ...
    print(f"📤 Alert on every scan: {CONFIG['ALERTS'].get('alert_on_every_scan', True)}")
    print()
...
async def main():
    ...
    print("="*70)
    print("🚀 ARBITRAGE BOT - CONTINUOUS ALERTS")
    print("="*70)
    print("📊 Exchanges: Binance, Bybit, OKX, KuCoin, Kraken, Gate.io")
    print("📤 Alerts: EVERY opportunity found (with cooldown for duplicates)")
