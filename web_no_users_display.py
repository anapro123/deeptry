"""
ARBITRAGE BOT - USERS HIDDEN
Users saved to users.txt - Not displayed on web page
"""
...
USERS_FILE = 'users.txt'  # Separate file for users
...
def save_user(name, chat_id):
    """Save a single user to users.txt"""
...
# Load users into CONFIG for backward compatibility
...
class AlertSystem:
    ...
    async def send_telegram(self, message, chat_id=None):
        ...
        if chat_id:
            ...
            users = load_users()  # Load fresh from file
...
class ArbitrageEngine:
    ...
    async def scan(self):
        ...
        if alerts_sent > 0:
            print(f"📤 Sent {alerts_sent} alerts for scan {self.scan_count}")
...
# ============================================
# FLASK WEB APP - NO USERS DISPLAY
# ============================================
...
def register():
    ...
    # Check if user exists using the file
...
# ============================================
# VIEW USERS (Admin only - via terminal)
# ============================================

def view_users():
    """Print users to terminal (admin only)"""
    users = load_users()
    print("\n" + "="*50)
    print("📋 REGISTERED USERS (Admin View)")
    print("="*50)
    if users:
...
async def background_scanner():
    ...
    # Show users in terminal (admin only)
    view_users()
    print()
