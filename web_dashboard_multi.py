"""
ARBITRAGE BOT WITH 6 EXCHANGES
    ...
    async def get_prices(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                ...
                prices = {}
                if data.get('result'):
                    for symbol, item in data['result'].items():
                        if any(symbol.endswith(base.lower()) for base in CONFIG['BASE_CURRENCIES']):
                            try:
                                prices[symbol.upper()] = {
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="confidence">{{ "%.0f"|format(opp.confidence) }}%</span></td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
