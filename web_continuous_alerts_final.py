...
Sends alerts EVERY scan, not just for new opportunities
"""
...
DEFAULT_CONFIG = {
    ...
    'ALERTS': {
        ...
        'cooldown_seconds': 2,  # Very short cooldown
        'max_alerts_per_scan': 30,
        'alert_on_every_scan': True,  # NEW: Alert every scan
    },
    ...
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            ...
            if 'ALERTS' not in config:
                config['ALERTS'] = DEFAULT_CONFIG['ALERTS']
            else:
                for key in DEFAULT_CONFIG['ALERTS']:
                    if key not in config['ALERTS']:
                        config['ALERTS'][key] = DEFAULT_CONFIG['ALERTS'][key]
            if 'TELEGRAM' not in config:
                config['TELEGRAM'] = DEFAULT_CONFIG['TELEGRAM']
            else:
                if 'bot_token' not in config['TELEGRAM']:
                    config['TELEGRAM']['bot_token'] = DEFAULT_CONFIG['TELEGRAM']['bot_token']
            save_config(config)
            return config
    else:
...
class AlertSystem:
    ...
    async def send_alert(self, opportunity):
        ...
        # VERY SHORT COOLDOWN - only prevent duplicate within 2 seconds
...
class ArbitrageEngine:
    ...
    async def scan(self):
        ...
        # SEND ALERTS FOR ALL OPPORTUNITIES (with short cooldown)
        if self.opportunities:
            max_alerts = CONFIG['ALERTS'].get('max_alerts_per_scan', 30)
            
            print(f"\n📤 Sending alerts for {min(len(self.opportunities), max_alerts)} opportunities...")
            
            for opp in self.opportunities[:max_alerts]:
                await self.alert_system.send_alert(opp)
                await asyncio.sleep(0.3)  # Small delay between alerts
            
            print(f"✅ Alert batch complete for scan {self.scan_count}")
...
<html>
<head>
    <title>🚀 Arbitrage Bot - Continuous Alerts</title>
    <meta http-equiv="refresh" content="10">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', sans-serif; background: #0a0a0a; color: #00ff00; padding: 15px; }
        ...
        .section h2 { color: #00ff00; font-size: 1.2em; margin-bottom: 10px; }
        .stats-grid {
            display: grid;
        ...
        .exchange-status .name { color: #00ff00; font-weight: bold; font-size: 0.9em; }
        .exchange-status .status { font-size: 0.7em; margin-top: 3px; }
        ...
        .register-box input {
            ...
            border-radius: 5px; color: #00ff00; font-size: 0.9em;
        }
        .register-box .btn {
        ...
        .how-to h3 { color: #00ff00; font-size: 0.9em; margin-bottom: 8px; }
        .how-to .step {
            display: flex;
            align-items: center;
            gap: 10px;
        ...
        .how-to .step-content p { color: #aaa; font-size: 0.8em; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 ARBITRAGE BOT</h1>
            <div class="subtitle">Real-time arbitrage scanning - Continuous Alerts</div>
            <div class="alert-status">🔔 Alerts sent every scan (cooldown: {{ config.ALERTS.cooldown_seconds }}s)</div>
        </div>
...
def register():
    ...
    return render_template_string(
        ...
        stats=engine.stats,
        config=CONFIG,
        message=f"✅ Welcome {name}! You'll receive alerts.",
        message_type="success"
    )
...
async def background_scanner():
    print("🔄 Scanner started - CONTINUOUS ALERTS")
    print(f"🎯 Min Spread: {CONFIG['MIN_PROFIT_PERCENT']}%")
    print(f"💰 Min Profit: ${CONFIG['MIN_PROFIT_USD']}")
    print(f"⏱️  Alert Cooldown: {CONFIG['ALERTS']['cooldown_seconds']}s")
    print()
...
async def main():
    ...
    print("📤 Alerts: Sent EVERY scan (with 2s cooldown)")
