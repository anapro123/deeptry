"""
ARBITRAGE BOT WITH SELF-SERVICE USER REGISTRATION
Users can add their own Telegram ID to receive alerts
"""
...
class AlertSystem:
    ...
    async def send_discord(self, message):
        ...
        for webhook in CONFIG['DISCORD']['webhooks']:
            if webhook.get('url'):
                data = {'content': f"```\n{message}\n```"}
                async with aiohttp.ClientSession() as session:
                    try:
                        await session.post(webhook['url'], json=data)
                        print(f"✅ Discord sent to {webhook['name']}")
                    except Exception as e:
...
# ============================================
# FLASK WEB APP WITH SELF-SERVICE
# ============================================
...
# HTML Template with User Registration Form
MAIN_TEMPLATE = '''
<!DOCTYPE html>
<html>
<head>
    <title>🚀 Arbitrage Bot - Get Alerts</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .alert-info { background: #1a1a3a; border: 1px solid #4444ff; color: #4444ff; }
        ...
        .user-card .name { color: #00ff00; font-weight: bold; }
        .user-card .id { color: #888; font-size: 0.8em; }
        ...
        .stat-item .value { font-size: 1.5em; color: #00ff00; }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="subtitle">Get real-time arbitrage alerts on Telegram</div>
        </div>
        ...
        <div class="section">
            ...
            <div class="register-box">
                <form method="POST" action="/register">
                    <input type="text" name="name" placeholder="Your Name (e.g., John)" required>
                ...
                <div class="how-to">
                    ...
                    <ol>
                        ...
                        <li>Copy the <strong>Chat ID</strong> number it sends you</li>
                        <li>Paste it in the field above and click "Get Alerts"</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="section">
            <h2>👥 Active Users ({{ config.TELEGRAM.users|length }})</h2>
            <div class="user-list">
                {% for user in config.TELEGRAM.users %}
                <div class="user-card">
                    <div class="name">{{ user.name }}</div>
                    <div class="id">ID: {{ user.chat_id }}</div>
                </div>
                {% else %}
        ...
        <div class="section">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px;">
                ...
                <div style="background: #1a1a1a; padding: 10px; border-radius: 5px; text-align: center; border: 1px solid #333;">
                    <div style="color: #00ff00; font-weight: bold;">{{ name.upper() }}</div>
                    <div style="color: {% if 'online' in status %}#00ff00{% else %}#ff4444{% endif %}; font-size: 0.8em;">
                        {{ status }}
                    </div>
                </div>
                {% endfor %}
    ...
    for user in CONFIG['TELEGRAM']['users']:
        if user['chat_id'] == chat_id:
            return render_template_string(
                ...
                stats=engine.stats,
                config=CONFIG,
                message=f"✅ {name}, you're already registered! You'll receive alerts.",
...
Stay tuned for opportunities! 🚀
        """
        await alert.send_telegram(welcome_msg, chat_id)
    
    # Run welcome in background
...
async def main():
    ...
    print("\n" + "="*70)
    print("🚀 ARBITRAGE BOT - SELF-SERVICE USER REGISTRATION")
    print("="*70)
    print("📊 Exchanges: Binance, Bybit, OKX, KuCoin, Kraken, Gate.io")
    print("🌐 Web: http://localhost:5000")
    print("📱 Users can register themselves!")
    print("="*70 + "\n")
