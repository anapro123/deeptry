"""
ARBITRAGE BOT - FORCE ALERTS
Sends EVERY opportunity found - NO FILTERS!
"""
...
DEFAULT_CONFIG = {
    'MIN_PROFIT_PERCENT': 0.0001,      # Almost zero
    'MIN_PROFIT_USD': 0.00001,          # Almost zero
    'MAX_PROFIT_PERCENT': 99999,        # No limit
    'TRADING_FEE': 0.001,
    ...
    'ALERTS': {
        'enabled': True,
        'min_profit_to_alert': 0.00001,  # Almost zero
        'min_confidence_to_alert': 0,     # No confidence filter
        'cooldown_seconds': 1,            # 1 second cooldown
        'max_alerts_per_scan': 50,        # Send up to 50 alerts
    },
...
class AlertSystem:
    ...
    async def send_alert(self, opportunity):
        ...
        # ONLY filter out obvious data errors
...
class ArbitrageEngine:
    ...
    async def scan(self):
        ...
        # FORCE SEND ALERTS - EVERY OPPORTUNITY
        if self.opportunities:
            max_alerts = CONFIG['ALERTS'].get('max_alerts_per_scan', 50)
            
            print(f"\n📤 Sending {min(len(self.opportunities), max_alerts)} alerts for scan {self.scan_count}...")
            
            for opp in self.opportunities[:max_alerts]:
                await self.alert_system.send_alert(opp)
                await asyncio.sleep(0.2)  # Small delay
            
            print(f"✅ Alerts sent for scan {self.scan_count}")
...
<html>
<head>
    <title>🚀 Arbitrage Bot - Force Alerts</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 ARBITRAGE BOT</h1>
            <div class="subtitle">FORCE ALERTS - Sending ALL opportunities</div>
            <div class="alert-status">🔔 Alerts: FORCED - Every opportunity sent!</div>
        </div>
...
def register():
    ...
    for user in CONFIG['TELEGRAM']['users']:
        if user['chat_id'] == chat_id:
            return f"✅ {name}, already registered!", 200
...
async def background_scanner():
    print("🔄 Scanner started - FORCE ALERTS MODE")
...
async def main():
    ...
    print("="*70)
    print("🚀 ARBITRAGE BOT - FORCE ALERTS")
    print("="*70)
    print("📊 Exchanges: Binance, Bybit, OKX, KuCoin, Kraken, Gate.io")
    print("📤 Alerts: FORCED - Sending EVERY opportunity")
