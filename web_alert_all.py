"""
ARBITRAGE BOT - ALERT FOR ALL OPPORTUNITIES
Sends alerts for EVERY opportunity found in EVERY scan
"""
...
DEFAULT_CONFIG = {
    ...
    'ALERTS': {
        'enabled': True,
        'min_profit_to_alert': 0.001,  # Almost zero - alert on ANY profit
        'min_confidence_to_alert': 0,   # No confidence filter
        'cooldown_seconds': 5,          # Very short cooldown
        'alert_on_every_scan': True,
        'max_alerts_per_scan': 20,      # Send up to 20 alerts per scan
    },
    ...
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            ...
            if 'MAX_PROFIT_PERCENT' not in config:
                config['MAX_PROFIT_PERCENT'] = 50
            if 'max_alerts_per_scan' not in config.get('ALERTS', {}):
                config['ALERTS']['max_alerts_per_scan'] = 20
            return config
    else:
...
# ============================================
# EXCHANGE APIS (SAME AS BEFORE - KEPT COMPACT)
# ============================================
...
# ============================================
# ALERT SYSTEM - SEND ALL OPPORTUNITIES
# ============================================
...
class AlertSystem:
    ...
    async def send_alert(self, opportunity):
        ...
        # Only filter out obvious data errors (ROI > 50%)
        if opportunity['roi'] > CONFIG['MAX_PROFIT_PERCENT']:
            return
        
        # ALMOST NO FILTERS - send everything!
        # Only check if profit is positive
        if opportunity['profit_usd'] <= 0:
            return
        
        # Very short cooldown just to prevent exact duplicates in same second
        key = f"{opportunity['pair']}_{opportunity['buy_exchange']}_{opportunity['sell_exchange']}"
        now = time.time()
        cooldown = CONFIG['ALERTS']['cooldown_seconds']
        
        # Only block if we sent the EXACT same opportunity in the last cooldown period
...
class ArbitrageEngine:
    ...
    async def scan(self):
        ...
        for pair, exchange_data in pairs.items():
            ...
            if net_spread > CONFIG['MAX_PROFIT_PERCENT']:
                continue
        ...
        filtered_opportunities = [o for o in opportunities if o['roi'] <= CONFIG['MAX_PROFIT_PERCENT']]
        
        self.opportunities = filtered_opportunities[:CONFIG.get('MAX_OPPORTUNITIES_DISPLAY', 100)]
        
        # SEND ALERTS - FOR EVERY OPPORTUNITY (up to 20 per scan)
        print(f"\n📊 Scan {self.scan_count}: Found {len(self.opportunities)} opportunities")
        
        # Send alerts for ALL opportunities (up to max_alerts_per_scan)
        max_alerts = CONFIG['ALERTS'].get('max_alerts_per_scan', 20)
        alerts_sent = 0
        ...
        if alerts_sent > 0:
            print(f"📤 Sent {alerts_sent} alerts for this scan")
...
<html>
<head>
    <title>🚀 Arbitrage Bot - All Alerts</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header .alert-status .highlight { color: #ff8800; font-weight: bold; }
        ...
        th { background: #1a1a1a; color: #00ff00; padding: 10px; text-align: left; border-bottom: 2px solid #00ff00; }
        ...
        .exchange-tag { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 0.7em; font-weight: bold; }
        ...
        .register-box { max-width: 600px; margin: 0 auto; text-align: center; }
        .register-box input {
            width: 100%; padding: 12px; margin: 8px 0;
            background: #1a1a1a; border: 1px solid #333;
            border-radius: 5px; color: #00ff00; font-size: 1em;
        }
        .register-box input:focus { border-color: #00ff00; outline: none; }
        .register-box .btn {
        ...
        .how-to h3 { color: #00ff00; margin-bottom: 15px; }
        .how-to .step {
            display: flex;
            align-items: center;
            gap: 15px;
        ...
        .how-to .step:hover { background: #111; }
        .how-to .step-number {
            background: #00ff00;
            color: #0a0a0a;
            width: 28px;
        ...
        .how-to .step-content { flex: 1; }
        .how-to .step-content p { color: #aaa; font-size: 0.9em; }
        ...
        .how-to .telegram-btn:hover { background: #0099dd; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 ARBITRAGE BOT</h1>
            <div class="subtitle">Sends alerts for EVERY opportunity found!</div>
            <div class="alert-status">
                🔔 Alerts: <span class="highlight">ALL OPPORTUNITIES</span> | 
                Cooldown: {{ config.ALERTS.cooldown_seconds }}s | 
                Max per scan: {{ config.ALERTS.max_alerts_per_scan }}
            </div>
        </div>
                ...
                <div class="how-to">
                    ...
                    <div class="step">
                        ...
                        <div class="step-content">
                            <p><strong>Open Telegram</strong> and search for <code>@id-bot</code></p>
                            <a href="https://t.me/id-bot" target="_blank" class="telegram-btn">📨 Open @id-bot</a>
                        </div>
                    </div>
                    ...
                    <div class="step">
                        ...
                        <div class="step-content">
                            <p>Click <strong>"Start"</strong> or send: <code>/start</code></p>
                        </div>
                    </div>
                    ...
                    <div class="step">
                        ...
                        <div class="step-content">
                            <p>📋 Copy the <strong>Chat ID</strong> number the bot sends you</p>
                            <p style="color: #666; font-size: 0.8em; margin-top: 3px;">Example: <code style="color: #00ff00;">123456789</code></p>
                        </div>
                    </div>
                    ...
                    <div class="step">
                        ...
                        <div class="step-content">
                            <p>✅ Paste it in the field above and click <strong>"Get Alerts"</strong></p>
                        </div>
                    </div>
                    
                    <div style="margin-top: 15px; padding: 12px; background: #0a0a0a; border-radius: 8px; border: 1px dashed #333;">
                        <p style="color: #666; font-size: 0.8em;">
                            💡 <strong>Tip:</strong> Your Chat ID is a number like <code style="color: #00ff00;">123456789</code>. 
                            It's unique to your Telegram account.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="section">
            <h2>👥 Registered Users ({{ config.TELEGRAM.users|length }})</h2>
            <div class="users-list">
                {% for user in config.TELEGRAM.users %}
                <span class="user-tag">✅ {{ user.name }}</span>
                {% else %}
                <span style="color: #666;">No users registered yet</span>
                {% endfor %}
            </div>
        </div>
        ...
        <div class="section">
            <h2>📊 Live Opportunities ({{ opportunities|length }})</h2>
            {% if opportunities %}
            <div class="opportunities-table">
                <table>
                    <thead>
                        <tr>
                            <th>#</th><th>Pair</th><th>Buy</th><th>Sell</th>
                            <th>Profit</th><th>ROI</th><th>Spread</th>
                            <th>Trade</th><th>Available</th><th>Confidence</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="{% if opp.profit_usd > 1 %}profit-mega{% elif opp.profit_usd > 0.1 %}profit-high{% else %}profit-positive{% endif %}">
                                ${{ "%.4f"|format(opp.profit_usd) }}
                            </td>
                            <td>{{ "%.2f"|format(opp.roi) }}%</td>
                            <td>{{ "%.3f"|format(opp.net_spread) }}%</td>
        ...
        <div class="section">
            ...
            <div class="exchange-status-grid">
                ...
                <div class="exchange-status">
                    ...
                    <div class="status">
                        <span class="{% if 'online' in status %}status-online{% else %}status-offline{% endif %}">
                            {{ status }}
                        </span>
                    </div>
                    <div class="pairs">{{ stats.exchanges_pairs.get(name, 0) }} pairs</div>
                </div>
                {% endfor %}
            </div>
        </div>
...
📊 You'll receive notifications for EVERY opportunity found.

🔔 Settings:
• Min Profit: ${CONFIG['ALERTS']['min_profit_to_alert']}
• Cooldown: {CONFIG['ALERTS']['cooldown_seconds']}s
• Max alerts per scan: {CONFIG['ALERTS']['max_alerts_per_scan']}
...
Get ready for opportunities! 🚀
    ...
    return render_template_string(
        ...
        stats=engine.stats,
        config=CONFIG,
        message=f"✅ Welcome {name}! You'll receive alerts for ALL opportunities.",
        message_type="success"
    )
...
@app.route('/api/users')
def api_users():
    return jsonify(CONFIG['TELEGRAM']['users'])
...
async def background_scanner():
    print("🔄 Scanner started - ALERT FOR ALL OPPORTUNITIES")
    print(f"🎯 Max ROI filter: {CONFIG['MAX_PROFIT_PERCENT']}%")
    print(f"💰 Min Profit: ${CONFIG['MIN_PROFIT_USD']}")
    print(f"⏱️  Cooldown: {CONFIG['ALERTS']['cooldown_seconds']}s")
    print(f"📤 Max alerts per scan: {CONFIG['ALERTS']['max_alerts_per_scan']}")
    print(f"👥 Users: {len(CONFIG['TELEGRAM']['users'])}")
    print()
...
async def main():
    ...
    print("="*70)
    print("🚀 ARBITRAGE BOT - ALERT FOR ALL OPPORTUNITIES")
    print("="*70)
    print("📊 Exchanges: Binance, Bybit, OKX, KuCoin, Kraken, Gate.io")
    print("📤 Alerts: EVERY opportunity found (up to 20 per scan)")
